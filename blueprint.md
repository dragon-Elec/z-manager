...
*   **Key Functions:**
    ...
    *   `update_fstab(path, uuid)`: Appends entry to `/etc/fstab` (safe append, backup first).
        *   **Robustness:** ALways uses `nofail` option (e.g., `UUID=... none swap sw,pri=0,nofail 0 0`) to prevent boot failures if device is missing.
    *   `configure_resume_device(device_path)`:
        *   **Purpose:** Configures the kernel's resume device for the *next* boot.
        *   **Mechanism:** Updates initramfs configuration (e.g. `conf.d/resume` or `dracut.conf.d`).
        *   **Note:** We do NOT write to `/sys/power/resume` at runtime for configuration; that interface is for triggering an immediate resume from a suspended image. Configuration must happen via bootloader/initramfs.
    ...
    *   `check_memory_fitness()`:
         *   **Pre-Hibernate Check:** Verifies if `Swap Total - Swap Used > (RAM Used - Cache)`.
         *   Prevents triggering hibernation if destination swap is too small to hold the image, avoiding kernel-panic or failed hibernation attempts.
