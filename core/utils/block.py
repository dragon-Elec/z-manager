...
from .swap import is_device_active
...
def check_device_safety(device_path: str) -> tuple[bool, str]:
    ...
    if fs_type := get_device_filesystem_type(device_path):
        return (
            False,
            f"Device {device_path} contains a '{fs_type}' filesystem. usage would cause data loss.",
        )
...
def get_device_scheduler(device_name: str) -> tuple[str, list[str]]:
    ...
    if not (
        content := Path(path).read_text(encoding="utf-8") if path.exists() else None
    ):
        return "unknown", []
...
def list_block_devices() -> list[dict[str, Any]]:
    ...
    try:
        res = run(
            ["lsblk", "-J", "-o", "NAME,PATH,SIZE,TYPE,LABEL,FSTYPE,MOUNTPOINT,MODEL"]
        )
        if res.code != 0:
            return []
        ...
        def recurse(dev_list):
            for d in dev_list:
                if (d.get("name") or "").startswith("zram"):
                    continue
