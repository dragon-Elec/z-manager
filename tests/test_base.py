...
class BaseTestCase(unittest.TestCase):
    ...
    def mock_system_calls(self, read_data=None, run_result=None):
        ...
        if read_data is not None:
            p = patch('core.utils.common.read_file', return_value=read_data)
            patchers.append(p)
            self.mock_read = p.start()
            
        if run_result is not None:
            p = patch('core.utils.common.run', return_value=MagicMock(code=0, out=run_result if isinstance(run_result, str) else ""))
            patchers.append(p)
