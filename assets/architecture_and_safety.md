...
> **Note:** This "Live Surgeon" logic is currently **bypassed** by the Configure Page in favor of the safer "Config Writer" approach (see below).
...
| `core/config_writer.py` | Generates the content string for `zram-generator.conf`. Provides `generate_config_string()` and `update_zram_config()`. Used by `zdevice_ctl.persist_writeback()`. |
...
*   **Where:** `ui/configure_page.py` -> `zdevice_ctl.apply_device_config()`
*   **What:** The Configure Page does **not** instantly apply changes.
...
| Function | Purpose |
| :--- | :--- |
| `get_psi(resource)` | Returns a snapshot `PsiStats` for a specific resource ('cpu', 'memory', 'io'). |
| `watch_psi(resource, interval)` | Generator that yields PSI snapshots for the resource at a given interval. |
...
| Function | Purpose |
| :--- | :--- |
| `list_zram_logs(unit, count)` | Returns a list of `JournalRecord` dataclasses. |
...
| :--- | :--- | :--- |
| `get_current_cpu_governor()` | `/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor` | Returns current governor (e.g., `powersave`). |
...
*   `__init__`: Calls `self.refresh()` and sets up a 1-second timer.
...
*   `AdwPreferencesGroup` "Safety Limits": `AdwSwitchRow` for host memory limit toggle, `AdwEntryRow` for resident memory limit (`zram-resident-limit`).
*   `AdwPreferencesGroup` "Advanced Settings": `AdwExpanderRow` containing filesystem mode toggle, writeback device selector, and `AdwEntryRow` for raw options.
...
*   `_populate_profiles()`: Creates `ScenarioCard` widgets from `profiles.get_all_profiles()`.
*   `on_select_writeback_clicked()`: Opens `DevicePickerDialog`.
*   `on_apply_clicked()`: Calculates pending changes, shows Confirmation Dialog, and starts a background thread (`_apply_worker`) to execute safe `zdevice_ctl` transactions.
...
### 6.8. Health Button: `ui/health_button.py`

**Class:** `HealthStatusButton(Gtk.Button)`

**Purpose:** A compact, persistent indicator of system health in the Status Page header.

**States:**
*   `HEALTHY` (Green Check): All systems nominal.
*   `WARNING` (Yellow Alert): Non-critical issues (e.g., zswap enabled, journal missing).
*   `ERROR` (Red X): Critical issues (e.g., sysfs inaccessible).
*   `UNKNOWN`: Initial state.

---

### 6.9. Health Dialog: `ui/health_dialog.py`

**Class:** `HealthReportDialog(Adw.Window)`

**Purpose:** A detailed report window shown when the Health Button is clicked.

**Features:**
*   **Overall Status Summary:** Large icon and description of the current state.
*   **Health Checks List:** Individual rows for ZRAM active status, Sysfs access, ZSwap conflict, Systemd availability, and Journal access.
*   **Diagnostics:** Button to open the Log Viewer.
*   **Notes & Warnings:** Specific actionable advice (e.g., "ZSwap is enabled - may interfere").

---

### 6.10. Log Viewer: `ui/log_viewer.py`

**Class:** `LogViewerDialog(Adw.Window)`

**Purpose:** An internal tool to view raw `zram-setup` logs without opening a terminal.

**Features:**
*   **Syntax Highlighting:** Uses `Gtk.TextTag` to colorize keywords (Error, Warning, Active/Success) and timestamps (dimmed).
*   **Filtering:** Automatically aggregates logs for all active ZRAM devices or identifying setup service units.
*   **Mock Terminal:** Styled to resemble `journalctl` output.
...
**Path:** `/etc/sysctl.d/99-z-manager.conf`
